...
Beyond CRUD operations, you can attach custom behavior to records using the `Method()` function. Record-level methods are defined when creating the model and become available on all record instances:
...
Record methods have access to `self`, which refers to the record instance. You can access the record's data via `self$data` and call other record methods like `self$update()` or `self$relationship()`.

For more complex examples of using methods for business logic, see the [Using Methods](using-methods.html) vignette.