...
- Reflection is **dialect-agnostic** and recovers column **names and
  best-effort types** only. Primary keys, nullability, defaults, and foreign
  keys are **not** reflected yet (a richer, per-dialect reflection is a natural
  follow-up).
- Because the primary key is not reflected, `update()` and `delete()` — which
  key off declared primary-key fields — require you to supply the key column
  via `...`. Arguments passed through `...` take precedence over reflected
  columns, mirroring `model()`:

```{r, eval=FALSE}
Users <- engine$hydrate(
  "users",
  id = Column("INTEGER", primary_key = TRUE),
  # ... can also attach Method()s or override reflected column types
)
```
