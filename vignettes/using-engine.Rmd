...
- `hydrate()` Builds a `TableModel` by introspecting an existing table, so you can do basic CRUD without declaring columns. See [Hydrating models from existing tables](#hydrating-models-from-existing-tables) below.

- `hydrate_schema()` Hydrates all tables in a schema at once and auto-wires relationships from reflected foreign keys. See [Hydrating an entire schema](#hydrating-an-entire-schema) below.
...
When a table already exists in the database, `hydrate()` inspects its columns
...
Foreign keys pointing at tables outside the hydrated set are silently skipped
